...
def ship_hit(ai_settings, stats, screen, ship, bullets):
    '''Обрабатывает столкновние корабля с пришельцем'''
    if stats.ship_left > 0:
        # уменьшает ship_left
        stats.ship_left -=1
        # очистка списков пришельцев и пуль
        bullets.empty()
        # создание нового флота и размешение корабля по центра
        update_bullets(ai_settings, stats, screen, ship, bullets)
        ship.center_ship()
    else:
        print('Конц игры')
...
def update_bullets(ai_settings, stats, screen, ship, bullets):
